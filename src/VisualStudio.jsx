...
export default function VisualStudio({ title, setTitle, subtitle, setSubtitle, image, setImage, prompt, setPrompt }){
 const canvasRef=useRef(null); const [theme,setTheme]=useState(0); const [exporting,setExporting]=useState(false); const [engine,setEngine]=useState('free'); const [key,setKey]=useState(''); const [endpoint,setEndpoint]=useState('https://ai.api.nvidia.com/v1/genai/stabilityai/stable-diffusion-xl'); const [status,setStatus]=useState(''); const [showOrbits, setShowOrbits] = useState(true);
 ...
 const draw=(ctx,w,h,t,bg)=>{
  const [dark,accent,light]=palettes[theme];
  ctx.fillStyle=dark;
  ctx.fillRect(0,0,w,h);
  if(bg){
   const scale = 1.0 + Math.sin(t) * 0.03;
   const nw = w * scale;
   const nh = h * scale;
   const nx = (w - nw) / 2;
   const ny = (h - nh) / 2;
  ...
  if(showOrbits){
   ...
   for(let r=0;r<5;r++){
    ...
    ctx.globalAlpha=.12+r*.06;
    ctx.lineWidth=3;
    ctx.ellipse(cx,cy,150+r*58,150+r*58,Math.sin(t+r)*.3,0,Math.PI*2);
    ctx.stroke()
   }
   ctx.globalAlpha=1;
   for(let i=0;i<50;i++){
    const a=i/50*Math.PI*2+t*(i%2?1:-1),rad=165+(i%5)*57;
    ctx.beginPath();
    ctx.fillStyle=i%3?light:accent;
    ctx.arc(cx+Math.cos(a)*rad,cy+Math.sin(a)*rad,3+(i%4),0,Math.PI*2);
    ctx.fill()
   }
  }
  ctx.fillStyle=accent;
  ctx.font='500 18px monospace';
  ctx.fillText('SIGNAL / VISUAL BRIEF',55,65);
  ctx.fillStyle=light;
  ctx.font='700 58px Arial';
  wrap(ctx,title.toUpperCase(),55,h-245,w-110,66);
  ctx.fillStyle='#aaa6ae';
  ctx.font='400 23px Arial';
  wrap(ctx,subtitle,55,h-120,w-110,31);
  ctx.fillStyle=light;
  ctx.font='500 16px monospace';
  ctx.fillText('LearnAiWithLalit',55,h-40)
 }
